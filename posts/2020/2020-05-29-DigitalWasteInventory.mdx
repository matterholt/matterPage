...
```
Hopefully we get a nicely printed dictionary of all file extension that we are able to work with. It is nice to see how many files that are living in the directory, It allows us to scan down through and pick off any files that we know for a fact that we don't need. Even with this info I am left asking more questions. Like the size of the files. and how to know what files are the most current and need to keep them? how to create test for scripts. Future post on the questions at hand and I'm sure there are more to ask. 
